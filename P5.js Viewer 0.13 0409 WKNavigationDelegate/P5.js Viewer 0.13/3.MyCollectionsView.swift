                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())]) {
                    ForEach(elements) { element in
                        if let thumbnailURL = self.getThumbnailURL(for: element) {
                            ...
                            NavigationLink(destination: WebContentView(sourceLocalURL: folderURL, title: element.title, author: element.author,p5id:element.p5id,createdAt:element.createdAt)) {
                                VStack {
