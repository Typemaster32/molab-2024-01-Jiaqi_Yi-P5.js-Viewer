...
func unzipContent(sourceURL: URL, completion: @escaping (URL?,[Bool]) -> Void) {
    ...
    print("------<<<<<<[unzipContent]>>>>>>------")
    print("[unzipContent]:sourceURL: \(sourceURL.lastPathComponent)")
    print("[unzipContent]:uniqueDestinationURL: \(uniqueDestinationURL.lastPathComponent)")
    ...
    do {
        ...
        if fileManager.fileExists(atPath: indexPath.path) {
            ...
            insertMetaTagInHtmlFile(fromHtmlFile: indexPath, tagToImport: zoomerMetaTag)
...
func removeP5SoundScriptTag(fromHtmlFile filePath: URL) {
    print("[unzipContent]-[removeP5SoundScriptTag]")
    do {
        ...
        if htmlContent == newHtmlContent {
            print("[unzipContent]-[removeP5SoundScriptTag]:No script tag for 'p5.sound.min.js' was found in the HTML file.")
        } else {
            // 3. Write the modified String back to the file
            try newHtmlContent.write(to: filePath, atomically: true, encoding: .utf8)
            print("[unzipContent]-[removeP5SoundScriptTag]:Successfully removed the script tag for 'p5.sound.min.js' from the HTML file.")
        }
    } catch {
        print("\(red)[unzipContent]-[removeP5SoundScriptTag]:An error occurred while processing the HTML file: \(error)\(red)")
    }
}
let zoomerMetaTag = "<meta name=\"viewport\" content=\"height=device-height, initial-scale=1.0, maximum-scale=1.0, user-scalable=no\">"
func insertMetaTagInHtmlFile(fromHtmlFile: URL, tagToImport: String) {
    print("[unzipContent]-[insertMetaTagInHtmlFile]")
    let htmlFilePath = fromHtmlFile
    do {
        // 1. Read the HTML file into a String
        ...
        if !htmlContent.contains(tagToImport) {
            ...
            if let headRange = htmlContent.range(of: "<head>") {
                ...
                print("[unzipContent]-[insertMetaTagInHtmlFile]:Successfully inserted the meta tag into the HTML file.")
            } else {
                print("[unzipContent]-[insertMetaTagInHtmlFile]:No <head> tag was found in the HTML file.")
            }
        } else {
            print("[unzipContent]-[insertMetaTagInHtmlFile]:The meta tag is already present in the HTML file.")
        }
    } catch {
        print("\(red)[unzipContent]-[insertMetaTagInHtmlFile]:An error occurred while processing the HTML file: \(error)\(red)")
    }
}
// It would cause sketch1981360 being blank!!!
func insertCssRulesInCssFile(folderPath: URL) {
    print("[unzipContent]-[insertCssRulesInCssFile]: Initiated.")
    ...
    do {
        ...
        if !cssContent.contains(cssRule) {
            ...
            print("[unzipContent]-[insertCssRulesInCssFile]:Successfully inserted the CSS rules into the CSS file.")
        } else {
            print("[unzipContent]-[insertCssRulesInCssFile]:The CSS rule is already present in the CSS file.")
        }
    } catch {
        print("\(red)[unzipContent]-[insertCssRulesInCssFile]:An error occurred while processing the CSS file: \(error)")
    }
}
func removeCommentsFromJSFiles(inDirectory directoryPath: URL) {
    print("[unzipContent]-[removeCommentsFromJSFiles]: Initiated.")
    let fileManager = FileManager.default
    do {
        ...
        print("[unzipContent]-[removeCommentsFromJSFiles]:Comments removed from all .js files.")
    } catch {
        print("\(red)[unzipContent]-[removeCommentsFromJSFiles]:An error occurred: \(error)\(red)")
    }
}
func processJSFiles(inDirectory directoryURL: URL) {
    print("[unzipContent]-[processJSFiles]:Initiated.")
//    print("[unzipContent]-[processJSFiles]:Initiated at directoryURL:\(directoryURL)")
    let fileManager = FileManager.default
    ...
    do {
        ...
        for fileURL in jsFileURLs {
            ...
            print("[unzipContent]-[processJSFiles]:Processing JS file: \(fileURL.lastPathComponent)")
        ...
        print("\(red)[unzipContent]-[processJSFiles]:Error listing .js files: \(error)\(red)")
    }
}
func searchJSFilesForKeywords(inDirectory directoryURL: URL, keywords: [Any]) -> [Bool] {
    print("[unzipContent]-[searchJSFilesForKeywords]: Initiated.")
//    print("[unzipContent]-[searchJSFilesForKeywords]: Initiated at directoryURL:\(directoryURL)")
    let fileManager = FileManager.default
    ...
    do {
        ...
        print("\(red)[unzipContent]-[searchJSFilesForKeywords]:An error occurred: \(error)\(red)")
    }
    
    return results
}
func deleteSpecificFiles(inDirectory directoryURL: URL) {
    print("[unzipContent]-[deleteSpecificFiles]: Initiated.")
//    print("[unzipContent]-[deleteSpecificFiles]: Initiated at directoryURL:\(directoryURL)")
    ...
    filesToDelete.forEach { fileName in
        ...
        if fileManager.fileExists(atPath: fileURL.path) {
            do {
                ...
                print("[unzipContent]-[deleteSpecificFiles]:\(fileName) was successfully deleted.")
            } catch {
                print("\(red)[unzipContent]-[deleteSpecificFiles]:Could not delete \(fileName): \(error)")
            }
        } else {
            print("\(red)[unzipContent]-[deleteSpecificFiles]:\(fileName) does not exist in the specified directory.")
        }
    }
}
//There are two test functions:
func printZipFileDetails(url: URL) {
    print("[TEST]-[deleteSpecificFiles]: Initiated.")
//    print("[TEST]-[deleteSpecificFiles]: Initiated at url:\(url)")
    let fileManager = FileManager.default
    do {
        ...
        print("[unzipContent]-[printZipFileDetails]:File Name: \(fileName)")
        if let fileSize = fileSize {
            print("[unzipContent]-[printZipFileDetails]:File Size: \(fileSize.intValue) bytes")
        }
    } catch {
        print("\(red)[unzipContent]-[printZipFileDetails]:Error retrieving file attributes: \(error.localizedDescription)\(red)")
    }
}
func printFilesInFolder(url: URL) {
    print("[TEST]-[printFilesInFolder]: Initiated.")
//    print("[TEST]-[printFilesInFolder]: Initiated at url:\(url)")
    let fileManager = FileManager.default
    do {
        ...
        for fileURL in fileURLs {
            ...
            print("[TEST]-[printFilesInFolder]:File Name: \(fileName)")
            if let fileSize = fileSize {
                print("[TEST]-[printFilesInFolder]:File Size: \(fileSize.intValue) bytes")
            }
        }
    } catch {
        print("\(red)[TEST]-[printFilesInFolder]:Error listing directory contents: \(error.localizedDescription)\(red)")
    }
}
