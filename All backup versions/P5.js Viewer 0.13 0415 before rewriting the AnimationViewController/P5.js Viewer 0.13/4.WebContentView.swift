...
//    
    ...
    func publishAction(title: String, author: String, unzippedContentURL: URL?) {
        print("[WebContentView][publishAction]")
        guard let unzippedURL = unzippedContentURL else {
            print("[WebContentView][publishAction]: Invalid URL provided.")
            return
        }
        ...
        if !viewModelFirebase.isPublished {
            captureCanvasForPublish(title: title, author: author, originalHtmlURL: unzippedURL) { savedImageURL in
                guard let savedImageURL = savedImageURL else {
                    print("[WebContentView][publishAction]: Failed to save the preview image.")
                    return
                }
                uploadImageToFirebaseStorage(imageURL: savedImageURL)
                let dateFormatter = DateFormatter()
                ...
                let thisItem = Item(
                    ...
                    createdAt: self.createdAt,
                    updatedAt: dateString,
                    previewName: savedImageURL.lastPathComponent
                )
                ...
                addDocument(thisItem) { updatedItem in
                    if let updatedItem = updatedItem {
                        ...
                        print("[WebContentView][publishAction][addDocument]: Failed to add the item.")
                    }
                }
            }
        } else {
            print("This Sketch is already Published")
        }
        viewModelFirebase.isPublished = true
    }

    
    private func captureCanvasForSavingPhoto() {
        print("[WebContentView][captureCanvasForSavingPhoto]")
        guard let webView = webViewStore.webView else {
            print("[WebContentView][captureCanvasForSavingPhoto]:returned 1")
            return }
        webViewStore.coordinator?.captureCanvasImage(webView: webView) { image in
            guard let image = image else {
                print("[WebContentView][captureCanvasForSavingPhoto]:returned 2")
                return
            }
            saveImage(image)
        }
    ...
    private func captureCanvasForCollection() {
        print("[WebContentView][captureCanvasForSavingPhoto]")
        guard let webView = webViewStore.webView else {
            print("[WebContentView][captureCanvasForSavingPhoto]:returned 1")
            return }
    ...
    private func captureCanvasForPublish(title: String, author: String, originalHtmlURL: URL, completion: @escaping (URL?) -> Void) {
        guard let webView = webViewStore.webView else {
            print("[WebContentView][captureCanvasForPublish]: Failed to access webView.")
            completion(nil)
            return
        }
        ...
        webViewStore.coordinator?.captureCanvasImage(webView: webView) { image in
            guard let image = image else {
                print("[WebContentView][captureCanvasForPublish]: No image captured.")
                completion(nil)
                return
            }
            let resizedImage = self.resizeImage(image)  // Assuming resizeImage handles the desired dimensions
            guard let imageData = resizedImage?.jpegData(compressionQuality: 0.8) else {
                print("[WebContentView][captureCanvasForPublish]: Failed to convert image to JPEG.")
                completion(nil)
                return
            }
            ...
            let safeAuthor = author.replacingOccurrences(of: " ", with: "-")
            let filename = "\(safeAuthor)-\(safeTitle).jpg"
            let fileURL = self.getTemporaryFileURL(for: filename)
            
            do {
                try imageData.write(to: fileURL, options: .atomic)
                completion(fileURL)
            } catch {
                print("[WebContentView][captureCanvasForPublish]: Error saving image to URL: \(error.localizedDescription)")
                completion(nil)
            }
        }
    }
    
    private func getTemporaryFileURL(for filename: String) -> URL {
        ...
        return temporaryFolder.appendingPathComponent(filename)
    }
    ...
    private func resizeImage(_ image: UIImage) -> UIImage? {
        var newImage = image
        let maxSize: CGSize = CGSize(width: 500, height: 500)
        ...
        let renderSize = CGSize(width: newWidth, height: newHeight)
        UIGraphicsBeginImageContextWithOptions(renderSize, false, image.scale)
