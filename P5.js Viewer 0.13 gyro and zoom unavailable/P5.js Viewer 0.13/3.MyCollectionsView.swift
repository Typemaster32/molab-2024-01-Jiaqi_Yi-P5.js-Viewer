...
struct CollectionsView: View {
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())]) {
                    ForEach(elements) { element in
                        if let thumbnailURL = self.getThumbnailURL(for: element) {
                            ...
                            NavigationLink(destination: WebContentView(sourceURL: folderURL, title: element.title, author: element.author)) {
                                VStack {
    ...
    private func getThumbnailURL(for element: ElementModel) -> URL? {
