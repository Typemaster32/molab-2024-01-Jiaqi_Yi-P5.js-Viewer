...
struct SearchView: View {
    ...
    var body: some View {
        NavigationView {
            VStack(alignment: .leading) {
                ...
                if !sketches.isEmpty {
                    List(sketches.indices, id: \.self) { index in
                        NavigationLink(destination: downloadSketchWithURL(urlString: "https://editor.p5js.org/editor/projects/\(sketches[index].id)/zip",author:currentAuthor,title:sketches[index].name,p5id:sketches[index].id,/*this id here should be p5id*/createdAt: sketches[index].createdAt)) {
            ...
            .onAppear {
                    // Perform an initial search if the query isn't empty when the view appears
               searchArtist()
            }
        }
    ...
    private var searchSection: some View {
        HStack {
            TextField(self.searchPlaceholder, text: $searchText)
                ...
                .submitLabel(.search)
                .onSubmit {
                    print("[SearchView][searchSection]Search has been submitted")
                    searchArtist()
                }
            
            Button(action: {
                print("[SearchView][searchSection]Search button has been clicked")
                searchArtist()
            }) {
    ...
    func searchArtist() {
        var textToBeActuallySearched = searchText
        if viewModelSearch.searchQuery != "" {
            textToBeActuallySearched = viewModelSearch.searchQuery
        }
        ...
        if !searchText.isEmpty && !searchHistory.contains(searchText) {
            ...
            if searchHistory.count > 5 {
                ...
                print("[SearchView][searchArtist]The last of search history is removed")
            }
        }

        // Assuming 'fetchSketchArray' fetches sketches and updates 'sketches' state
        fetchSketchArray(from: "https://editor.p5js.org/editor/\(textToBeActuallySearched)/projects") { result in
            DispatchQueue.main.async {
                switch result {
                case .success(let fetchedSketches):
                    self.sketches = fetchedSketches
                    currentAuthor = textToBeActuallySearched
                case .failure(let error):
                    print("\(red)[SearchView][searchArtist]:Error fetching sketches: \(error.localizedDescription)")
...
//        searchText = "Sketch of the artist" // Clear the search text
        self.searchPlaceholder = textToBeActuallySearched
        viewModelSearch.searchQuery = ""
    }
    ...
    let task = URLSession.shared.dataTask(with: url) { data, response, error in
        ...
        do {
            ...
            do {
                ...
                print("\(red)[fetchSketchArray]:Error response not in expected format or different error")
            } catch {
                print("\(red)[fetchSketchArray]:Failed to decode error response")
            }
            // Original error handling logic if the specific error message is not detected
            print("\(red)[fetchSketchArray]:Decoding failed with error: \(error)")
            if let decodingError = error as? DecodingError {
                ...
                case .dataCorrupted(let context),
                    ...
                    print("\(red)[fetchSketchArray]:Decoding error: \(context.debugDescription)")
                    print("[fetchSketchArray]:CodingPath: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))")
                @unknown default:
                    break
                }
            }
