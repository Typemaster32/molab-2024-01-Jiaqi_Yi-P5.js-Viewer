...
class ImageSaver: NSObject { // This is saving the photo, preview and thumbnail
    ...
    func saveImage(_ image: UIImage, compress: Bool = false, toURL url: URL? = nil, resizeToWidth width: CGFloat? = nil) {
        if let url = url {
            DispatchQueue.global(qos: .background).async {
                ...
                if let imageData = imageData {
                    do {
                        ...
                        print("Image saved successfully to URL: \(url)")
                    } catch {
...
//class ImageSaver: NSObject {// This is saving the image from the WKWebView, including thumbnails, and previews.
//    static let shared = ImageSaver()
...
//
//
//extension UIImage {// This is in charge of put the image into photo album
