...
struct WebContentView: View {
    ...
    @StateObject var animationViewWController: AnimationViewController?
                ...
                if viewModel.isLoading {
                    ...
                    VStack {
                        ...
                        HStack {// Publish if it is not published; Like if it is published;
                            Button(action: {
                                if !viewModel.isPublished {
                                    ...
                                    if didIPressedLike {// Unlike
                                        UnlikeAction(likesManager: likesManager, p5id: p5id, viewModel: viewModel)
                                    } else {// Like
                                        LikeAction(likesManager: likesManager, p5id: p5id, viewModel: viewModel)
                                    }
                                }
                            }) {
                                if !viewModel.isPublished {
                            ...
                            }.font(.system(size: 14, weight: .bold)).foregroundColor(viewModel.isPublished ? .blue : .green).padding(15)
    ...
    func saveAnimationLocalAction() {
        ...
        if let unzippedURL = unzippedContentURL {
            ...
            animationViewWController = AnimationViewController(sketchTitle: title, author: author, folderURL: originalFolderURL)
        } else {
